...
function frequentVisitors(){
    ...
    echo "$visitsPerDay" | while read -r line; do
        local hourRemoved=$(echo "$line" | cut -d " " -f 2 | cut -d ":" -f 1)
        local ip=$(echo "$line" | cut -d " " -f 1)
        echo "$ip $hourRemoved" >> freqtemp.txt
    done

    cat freqtemp.txt | sort -n | uniq -c | while read -r count ip day; do
        if [[ "$count" -gt 10 ]]; then
            echo "$count $ip $day"
        fi
    done
...
do
	...
	else echo "Invalid input. Enter a number 1 - 7."
